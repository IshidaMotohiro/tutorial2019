## XLConnect は Java がインストールされていることを前提とします

install.packages("XLConnect", dependencies = TRUE)
library(XLConnect)
## 上記を実行してエラーになる場合は R　を一旦終了し
# Macではアプリケーションのターミナルを
# WindowsではWindowsシステムツールのコマンドプロンプトを右クリックしてか管理者として
# 起動し、以下のように入力します
# Mac # sudo R CMD javareconf       # パスワードが要求されます
# Windows # C:¥Program Files¥R¥bin¥R CMD javareconf
# その上でRを起動してもう一度 install.packages("XLConnect", dependencies = TRUE) を実行
 ...
 for (i in seq_along(files)){
   tmp_file_name <- str_remove(files[i], " ")
   学生番号 <- stringi::stri_trans_nfkc_casefold(tmp_file_name) %>%
     str_extract("\\d{8,10}\\p{Han}*.xlsx$")
   print(学生番号)
 }
 
 
 #ファイル名から学生番号と名前を分離して抽出
 files %>% walk(
   {
